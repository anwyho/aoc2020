...
#         ] for state in list_of_states
#     ]
# result = (lambda f: (lambda x: f(f, x)))(lambda g, iter: [''] if iter == 0 else [state for list_of_states in [[c + n for n in g(g, iter - 1)] for c in ('.', 'L', '#')] for state in list_of_states])(16)
#
# open('/Users/alo/go/src/aoc/state_chunk_map_2.csv', 'w').write(
# #     print(
#     '\n'.join([
...
#                 ]
#             )
#         ) for state in (lambda f: (lambda x: f(f, x)))(lambda g, iter: [''] if iter == 0 else [state for list_of_states in [[c + n for n in g(g, iter - 1)] for c in ('.', 'L', '#')] for state in list_of_states])(16)
#     ])
# )
